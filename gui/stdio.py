...
_ = lambda x:x
#from i18n import _
from electrum_pot import mnemonic_encode, WalletStorage, Wallet
from electrum_pot.util import format_satoshis, set_verbosity
from electrum_pot.bitcoin import is_valid
from electrum_pot.network import filter_protocol
...
class ElectrumGui:
    ...
    def __init__(self, config, network):
        ...
        if not storage.file_exists:
            print "Wallet not found. try 'electrum-pot create'"
            exit()
    ...
    def do_send(self):
        if not is_valid(self.str_recipient):
            print(_('Invalid Potcoin address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-pot setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-pot setconfig' to change your settings")
        return True
