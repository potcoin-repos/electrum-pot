...
SYSTEM_CONFIG_PATH = "/etc/electrum-pot.conf"
...
class SimpleConfig(object):
    ...
    def init_path(self):
        ...
        print_error( "electrum-pot directory", self.path)

    def set_key(self, key, value, save = True):
        if not self.is_modifiable(key):
            print "Warning: not changing key '%s' because it is not modifiable" \
                  " (passed as command line option or defined in /etc/electrum-pot.conf)"%key
            return
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-pot.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-pot.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-pot.conf into user_config[]."""
